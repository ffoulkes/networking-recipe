...
class ConfigEncapTableTest : public BaseTunnelInfoTest {
 public:
  ConfigEncapTableTest() {}
  virtual ~ConfigEncapTableTest() = default;
};

TEST_F(ConfigEncapTableTest, successEncapTableEntryFailure) {
...
TEST_F(ConfigEncapTableTest, successEncapTableEntrySuccess) {
  struct tunnel_info tunnel_info = {0};
  InitV4TunnelInfo(tunnel_info);
  InitVxlanTagged(tunnel_info);
