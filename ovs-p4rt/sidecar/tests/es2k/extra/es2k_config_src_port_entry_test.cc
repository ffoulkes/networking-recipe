...
class Es2kConfigSrcPortEntryTest : public ::testing::Test {
 protected:
  Es2kConfigSrcPortEntryTest() {}
  virtual ~Es2kConfigSrcPortEntryTest() = default;
 ...
 */
TEST_F(Es2kConfigSrcPortEntryTest, connectFailure) {
 ...
 */
TEST_F(Es2kConfigSrcPortEntryTest, getPipelineConfigFailure) {
  struct src_port_info port_info = {0};
...
/**
 * Exercises ConfigureVsiSrcPortTableEntry() error path.
 */
TEST_F(Es2kConfigSrcPortEntryTest, configVsiSrcPortEntryFailure) {
  constexpr char ERROR_MESSAGE[] = "ConfigureVsiSrcPortTableEntry";
  struct src_port_info port_info = {0};
  InitSrcPortInfo(port_info);
  ...
  EXPECT_CALL(client, getPipelineConfig)
      .WillOnce(
          DoAll(SetArgPointee<0>(expected_p4info), Return(absl::OkStatus())));
  EXPECT_CALL(client, sendReadRequest)
      .WillOnce(InvokeWithoutArgs(DummySendReadRequest));
...
/**
 * Exercises ConfigureVsiSrcPortTableEntry() happy path.
 */
TEST_F(Es2kConfigSrcPortEntryTest, configVsiSrcPortEntrySuccess) {
  struct src_port_info port_info = {0};
  InitSrcPortInfo(port_info);
